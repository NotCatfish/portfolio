...
export default function Navbar({ showResume, onNavigate }) {
          ...
          <div className="hidden md:flex gap-6 text-sm font-sans tracking-wide">
            {['about', 'work', 'contact'].map(item => (
              <a 
                ...
                className="hover:text-sakura transition-colors duration-300 cursor-pointer"
              >
                {t[item]}
              </a>
            ))}
            <button 
              onClick={(e) => handleItemClick(e, 'resume')}
            ...
            <div className="flex flex-col space-y-4 text-base font-sans tracking-wide">
              {['about', 'work', 'contact'].map(item => (
                <a 
                  ...
                  href={`#${item}`} 
                  onClick={(e) => handleItemClick(e, item)}
                  className="py-2 hover:text-sakura transition-colors duration-300 border-b border-ink-900/5 dark:border-snow-100/5 cursor-pointer"
                >
                  {t[item]}
                </a>
              ))}
