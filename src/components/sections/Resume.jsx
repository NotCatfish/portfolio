  ...
  return (
    <div className="pt-32 pb-20 px-6 md:px-12 mx-auto max-w-[900px] font-sans text-black/[0.67] dark:text-white/[0.67] [&_strong]:text-black/[0.85] dark:[&_strong]:text-white/[0.85] [&_strong]:font-semibold leading-relaxed">
      ...
      <ProgressiveBlock>
        ...
        <div className="flex flex-wrap items-center gap-x-2 text-[0.95rem] text-amber-700 dark:text-cyan-100 font-medium">
      ...
      {t.education.items.map((item, index) => (
        <ProgressiveBlock key={index} className="mt-5">
          <div className="mb-6">
            <h3 className="text-[1.15rem] mb-1.5 flex flex-col md:flex-row md:justify-between md:items-center text-amber-700 dark:text-cyan-100">
              <span>{item.school}</span>
              <span className="text-[0.85rem] font-medium text-amber-700 dark:text-cyan-100 bg-black/10 dark:bg-white/10 px-2.5 py-0.5 rounded-full mt-2 md:mt-0 whitespace-nowrap self-start md:self-auto">{item.date}</span>
            </h3>
        ...
        <ul className="list-none p-0 m-0 space-y-3">
          {t.skills.items.map((skill, index) => (
            <li key={index} className="text-[1rem]">
              <span className="text-amber-700 dark:text-cyan-100 font-semibold">{skill.category}:</span> {skill.text}
            </li>
          ))}
        </ul>
      </ProgressiveBlock>
      ...
      {t.certifications.items.map((cert, index) => (
        <ProgressiveBlock key={index} className="mt-5">
          <div className="mb-8">
            <h3 className="text-[1.15rem] mb-1 flex flex-col md:flex-row md:justify-between md:items-center text-amber-700 dark:text-cyan-100">
              <span>{cert.name}</span>
              <span className="text-[0.85rem] font-medium text-amber-700 dark:text-cyan-100 bg-black/10 dark:bg-white/10 px-2.5 py-0.5 rounded-full mt-2 md:mt-0 whitespace-nowrap self-start md:self-auto">{cert.date}</span>
            </h3>
