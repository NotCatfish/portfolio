...
export default function Resume() {
  ...
  return (
    <div className="pt-32 pb-20 px-6 md:px-12 mx-auto max-w-[900px] font-sans text-black/60 dark:text-white/60 [&_strong]:text-black/[0.85] dark:[&_strong]:text-white/[0.85] [&_strong]:font-semibold leading-relaxed">
      ...
      <ProgressiveBlock>
        ...
        <div className="flex flex-wrap items-center gap-x-2 text-[0.95rem] text-black/60 dark:text-white/60">
        ...
        <ProgressiveBlock key={index} className="mt-5">
          <div className="mb-6">
            <h3 className="text-[1.15rem] mb-1.5 flex flex-col md:flex-row md:justify-between md:items-baseline text-cyan-600 dark:text-cyan-100">
              ...
              <span className="italic text-[0.95rem] text-black/60 dark:text-white/60 font-normal mt-1 md:mt-0">{item.date}</span>
            </h3>
          ...
          <div key={index}>
            <ProgressiveBlock>
              <h3 className="text-[1.15rem] mb-1 flex flex-col md:flex-row md:justify-between md:items-baseline text-cyan-600 dark:text-cyan-100">
                <span>{exp.role}</span>
                <span className="italic text-[0.95rem] text-black/60 dark:text-white/60 font-normal mt-1 md:mt-0">{exp.date}</span>
              </h3>
      ...
      {t.certifications.items.map((cert, index) => (
        <ProgressiveBlock key={index} className="mt-5">
          <div className="mb-8">
            <h3 className="text-[1.15rem] mb-1 flex flex-col md:flex-row md:justify-between md:items-baseline text-cyan-600 dark:text-cyan-100">
              <span>{cert.name}</span>
              <span className="italic text-[0.95rem] text-black/60 dark:text-white/60 font-normal mt-1 md:mt-0">{cert.date}</span>
            </h3>
